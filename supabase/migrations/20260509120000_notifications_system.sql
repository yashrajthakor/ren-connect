-- REN Notification System (in-app)
...
BEGIN
  ...
  IF norm_target = 'all' THEN
    SELECT array_agg(DISTINCT ur.user_id) INTO recipient_ids FROM public.user_roles ur;
  ELSE
    SELECT array_agg(DISTINCT ur.user_id) INTO recipient_ids
  ...
  IF coalesce(array_length(recipient_ids, 1), 0) > 0 THEN
    ...
    SELECT uid, 'announcement', _title, _body, _link,
...
DECLARE
  ...
  receiver_name text;
BEGIN
  SELECT name INTO giver_name FROM public.members WHERE user_id = NEW.giver_id LIMIT 1;
  SELECT name INTO receiver_name FROM public.members WHERE user_id = NEW.receiver_id LIMIT 1;

  IF TG_OP = 'INSERT' THEN
    INSERT INTO public.notifications(user_id, type, title, body, link, metadata)
    VALUES (NEW.receiver_id, 'lead_received',
      'New lead received',
      coalesce(giver_name, 'A member') || ' shared a lead: ' || NEW.lead_name,
      '/dashboard/leads',
  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    IF NEW.status = 'business_closed' THEN
      INSERT INTO public.notifications(user_id, type, title, body, link, metadata)
      VALUES (NEW.giver_id, 'business_closed',
        'Business closed',
        coalesce(receiver_name, 'Member') || ' closed business on lead: ' || NEW.lead_name,
        '/dashboard/leads',
        jsonb_build_object('lead_id', NEW.id, 'amount', NEW.closure_amount));
    ELSE
      INSERT INTO public.notifications(user_id, type, title, body, link, metadata)
      VALUES (NEW.giver_id, 'lead_updated',
        'Lead status updated',
