-- Expose user_id on the Valuable Members roster RPC. Leads (giver_id,
-- receiver_id) and 1:1 Feed logs (meeting_by_user_id, meeting_with_user_id)
-- are keyed on auth.users.id, not members.id — the admin From/To/Member
-- filters need this to match a Valuable Member against their leads/logs.
-- Return shape changes, so the function must be dropped first.
